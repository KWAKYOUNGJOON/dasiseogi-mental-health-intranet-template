...
### 운영 메모
- CRI의 최종 `resultLevel` 계산식은 서버 전용 규칙을 그대로 유지한다.
- 통계 경고 연결은 최종 결과레벨 코드만 사용하고, 개별 문항 기반 `CRITICAL_ITEM` 은 추가하지 않는다.
- `resultLevelCodes` 는 화면 표시 label 이 아니라 내부 code(`A`~`E`) 기준으로만 매칭한다.

### 통계 alert 매핑
- `A` → `HIGH_RISK` : `CRI 결과 A: 극도의 위기`
- `B` → `HIGH_RISK` : `CRI 결과 B: 위기`
- `C` → `CAUTION` : `CRI 결과 C: 고위험`
- `D` → `CAUTION` : `CRI 결과 D: 주의`
- `E` → 경고 생성 없음
...
```json
{
  "scaleCode": "CRI",
  "alertRules": [
    { "code": "CRI_RESULT_A", "type": "HIGH_RISK", "message": "CRI 결과 A: 극도의 위기", "resultLevelCodes": ["A"] },
    { "code": "CRI_RESULT_B", "type": "HIGH_RISK", "message": "CRI 결과 B: 위기", "resultLevelCodes": ["B"] },
    { "code": "CRI_RESULT_C", "type": "CAUTION", "message": "CRI 결과 C: 고위험", "resultLevelCodes": ["C"] },
    { "code": "CRI_RESULT_D", "type": "CAUTION", "message": "CRI 결과 D: 주의", "resultLevelCodes": ["D"] }
  ]
}
```
...
```json
{
  "items": [
    ...
    { "scaleCode": "CRI", "scaleName": "정신과적 위기 분류 평정척도 (CRI)", "displayOrder": 9, "isActive": true }
  ]
}
```
